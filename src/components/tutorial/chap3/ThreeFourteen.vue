...
export default {
  ...
  mounted() {
    ...
    system.addStave({ voices: [score.voice(score.notes('(G3 B3 D4)/h, A3/q', { clef: 'bass' }))] })
    ...
    system.addStave({
      voices: [
        score.voice([
          score.notes('D5/q,G4,G4'),
        ].reduce((a,b)=>{return a.concat(b)}))
      ],
    })
    system.addStave({ voices: [score.voice(score.notes('B3/2.', { clef: 'bass' }))] });
    system.addConnector('singleRight');
    system.addConnector('singleLeft');
